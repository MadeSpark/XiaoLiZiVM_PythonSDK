...
| `on_enable()` | 插件启用 | 无参数，返回 0=正常 / 1=拒绝启用 |
| `on_disable()` | 插件禁用 | 无参数 |
| `on_uninstall()` | 插件卸载 | 无参数 |
| `on_setting()` | 用户点击插件设置 | 无参数 |
...
| `on_event_message(ev)` | 其他事件 | `this_qq`, `event_type`, `event_sub_type`, `trigger_qq`, `source_group_qq`, `operate_qq`, `message` |
